...
pub fn find(summary: &str, candidates: &[Task]) -> Match {
    ...
    // choice the reporter has to make: they fall through to the similar list, where their
    // score of 1.0 sorts them first.
    if exact.len() == 1 {
...
mod tests {
    ...
    fn exact_versus_similar_versus_none() {
        ...
        match find("check rejects missing spec", &[spec.clone(), twin]) {
            Match::Similar(candidates) => assert_eq!(candidates.len(), 2),
