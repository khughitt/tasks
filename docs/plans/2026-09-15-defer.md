...
- Conventional commits, no attribution trailers. Task ids in messages: this plan implements `tasks-be6fcc`.
...
**Interfaces:**
- Consumes: `crate::periodic::Interval::parse`, `crate::model::{Status, Task}` (Task gains `defer` in Task 2; this task's predicates compile against it, so Task 1 and Task 2 are committed together — see Task 2 step 6).
- Produces:
  - `pub struct Defer(Date)` — `Defer::parse(&str) -> Result<Defer>`, `Defer::resolve(&str, today: Date) -> Result<Defer>`, `date(self) -> Date`, `Display` as `YYYY-MM-DD`, `Serialize` as that string, `Copy, PartialEq, Eq, PartialOrd, Ord`.
...
pub fn is_deferred(task: &Task, now: OffsetDateTime) -> bool {
    task.defer.is_some_and(|defer| now.date() < defer.date())
}
...
Expected: compile error, `no field \`defer\` on type \`Task\`` — the field lands in Task 2. Proceed to Task 2 before committing.
...
- Modify: `src/format.rs:6-31` (`KEYS`), `:121` (`parse_task`), `:152-168` (`quote_timestamps`), `:279-312` (`validate_task`), `:373` (`serialize_task`), tests at `:510-548`
...
Add `defer: None,` after `every: None,` (or the `every:` line) in each literal listed above. Compile errors name any missed site: `just check` must be clean before step 2.
...
`parse_task`, after the `every:` field:
...
`quote_timestamps`: add `|| line.starts_with("defer: ")` to the condition — a bare `2026-11-10` is a YAML timestamp scalar exactly as the RFC 3339 fields are (§3).
...
```bash
git add src/defer.rs src/main.rs src/model.rs src/format.rs src/commands/add.rs src/periodic.rs src/query.rs src/hierarchy.rs src/complexity.rs src/repo.rs src/similarity.rs
git commit -m "feat(defer): the defer date field and its predicates"
```
...
**Interfaces:**
- Consumes: `crate::defer::{Defer, can_carry}` (Task 1).
- Produces: `FieldArgs.defer: Option<String>`, `EditArgs.no_defer: bool`, `hierarchy::validate_defer(project, registry, task) -> Result<()>`, `complete::defer_dates()`.
...
- [ ] **Step 4: Implement the goal refusals**
...
Run: `just test-fast defer_`
Expected: all three PASS. Then `just test-fast` — the full inner loop passes (the `every`-on-goal tests still pass with the shared read).

- [ ] **Step 6: Commit**

```bash
git add src/cli.rs src/complete.rs src/commands/mod.rs src/commands/edit.rs src/hierarchy.rs src/repo.rs tests/cli.rs
...
**Interfaces:**
- Produces: `pub struct DeferredInfo { pub until: String, pub due: bool }` with `DeferredInfo::of(task, now) -> Option<DeferredInfo>`; field `deferred: Option<DeferredInfo>` on `TaskSummary`, `ShowFields`, and `ParkedRow`.
...
```bash
git add src/output.rs src/commands/show.rs tests/cli.rs
...
- Modify: `src/commands/list.rs:186-248` (`ready_tasks` returns `Picked`), `:262` (`ready`), `:289-305` (`next`), `:355` (`prime`)
...
**Interfaces:**
- Consumes: `crate::defer::is_deferred`, `DeferredInfo.due` on `ParkedRow` (Task 5).
- Produces:
  - `pub struct Picked { pub tasks: Vec<Task>, pub deferred: Vec<Task> }` in `src/query.rs`
  ...
  - `pub fn is_actionable(task, now) -> bool` now gated by the deferral (§4.1)
...
Run: `just test-fast deferred_work_is_hidden`, `just test-fast a_due_deferral`, `just test-fast deferred_parked_idea`, `just test-fast quiet_judges`, `just test-fast holds_its_dependents`
Expected: the first four FAIL (deferred records are listed, no warning); the last PASSES already (an open dependency holds regardless) and stays as the §2.3 guard.
...
pub fn readiness(
    ...
    {
        Readiness::Not
    } else if crate::defer::is_deferred(task, now) {
        Readiness::Deferred
    } else {
        Readiness::Ready
    }
...
(declare `let mut deferred = Vec::new();` beside `ready`). Keep the sort and the two `retain`s on `ready`; finish with `Ok(Picked { tasks: ready, deferred })`.

`ready` (`:262`):

```rust
    let picked = ready_tasks(&mut ctx, &all, &claims, now)?;
...
```bash
git add src/query.rs src/commands/list.rs src/commands/parked.rs src/commands/quiet.rs src/commands/sample.rs tests/cli.rs
...
**Interfaces:**
- Produces: `list --deferred`; `PrimeOut.deferred: DeferredSummary { waiting, next, in_days, due }`; the `defer <date>` / `due <date>` row marker; the `# deferred` block in `show --pretty`.
...
```bash
git add src/cli.rs src/commands/mod.rs src/commands/list.rs src/output.rs tests/cli.rs
...
Then, from the worktree (prefix routing reaches the other projects):
...
tasks done tasks-be6fcc "defer date: --defer/--no-defer, pickers skip it with one warning, list --deferred, prime line, check findings; prism-49a068 and prism-8a8eac moved onto the field"
```

If either prism record's body still says `revisit 2026-11-10` in prose, leave the prose — the note is history — but confirm `tasks show` reports `deferred.until` for each. Then `tasks check`, and commit the record changes in this repository:

```bash
git add tasks/
git commit -m "chore(tasks): close tasks-be6fcc and its feedback report"
```
